...
/*   Updated: 2025/02/24 17:23:01 by rbaldin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	mandelbroting(int x, int y, t_fractalid *f)
{
	...
	z.x = f->start_x;
	z.y = f->start_y;
	c.x = f->focus * remapping(x, -2, +2, W);
	c.y = f->focus * remapping(y, -2, +2, H);
...
static	void	julianing(int x, int y, t_fractalid *f)
{
	...
	c.x = f->start_x;
	c.y = f->start_y;
	z.x = f->focus * remapping(x, -2, +2, W);
	z.y = f->focus * remapping(y, -2, +2, H);
	while (i < f->resolution_index)
	{
		z = sum_z(square_z(z), c);
		if (z.x * z.x + z.y * z.y > f->outbound_value)
