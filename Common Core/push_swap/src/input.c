...
/*   Updated: 2025/01/15 10:24:14 by rbaldin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int	*numbers_parsing2(int argc, char **argv, int size);
...
t_stack	*input_checking(int argc, char **argv)
{
	...
	if (argc > 2)
		nums = numbers_parsing(argc, argv, &size);
	else if (argc == 2)
		nums = numbers_parsing2(argc, argv, &size);
	if (!nums || duplicated(nums, &size))
	{
		free(nums);
		nums = NULL;
		return (NULL);
	}
	stacka = stack_initializing(nums, &size);
	free(nums);
	if (!stacka)
		return (NULL);
	return (stacka);
...
static	char	**split_freeing(char **split)
{
	int	i;
