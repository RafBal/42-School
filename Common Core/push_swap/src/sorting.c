...
/*   Updated: 2025/01/26 12:04:56 by rbaldin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int	min_finding(t_stack **a)
{
	...
	while (a_reserv)
	{
		n_to_compare = *(int *)(long *)(a_reserv->content);
...
static	void	three_sort(t_stack *a, t_list *top, t_list *middle, t_list *bottom)
{
	...
	else if (topc > middlex && middlec > bottomc)
	{
		sa(&a);
		rra(&a);
	}
...
static	bool	sorted_already(t_stack *s)
{
	t_list	*now;
